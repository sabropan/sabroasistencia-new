...
with tab2:
    ...
    if not df_emp.empty:
        ...
        st.info("💡 Edite las celdas y presione 'Guardar Cambios en la Nube'.")
...
with tab3:
    ...
    if not df_f.empty:
        sel = st.selectbox("Seleccione Empleado:", [f"{r['biometric_id']} - {r['full_name']}" for _, r in df_f.iterrows()])
